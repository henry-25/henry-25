...
$(document).ready(function() {
    ...
    $.ajax({
		  ...
		  url: url + "start_date=" + startDateSelected + "&end_date=" + endDateSelected + "&api_key=" + apiKey,
		  	...
		  	for(i in neo) {
		  		for(j in neo[i]) {
		  			$("#asteroidDataTable > tbody:last-child").append("<tr><td>" + neo[i][j].name + "</td><td>" + neo[i][j].close_approach_data[0].close_approach_date + "</td><td>" + Math.floor(neo[i][j].close_approach_data[0].miss_distance.miles) + " miles</td><td>" + Math.floor(neo[i][j].close_approach_data[0].relative_velocity.miles_per_hour) + " mph</td></tr>");
		  		}
		  	}
